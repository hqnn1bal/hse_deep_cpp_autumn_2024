...
private:
	...
	std::function<void(std::uint64_t)> digitCallback;
...
public:
	...
	inline void SetDigitTokenCallback(std::function<void(std::uint64_t digit)> func) { this->digitCallback = func; }
	inline void SetStringTokenCallback(std::function<void(const std::string& str)> func) { this->stringCallBack = func; }
